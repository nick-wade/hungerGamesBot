...
function selectRandomVictim(player) {
    const availableVictims = Object.keys(players).filter(name => name !== player.name && !players[name].status.includes('dead'));
...
const selectCombatEvent = (player, ally=null) => {
    ...
    if (!ally){
        ...
        const eventsAlly = {
            "hatchet": {
                "decapitate": () => hatchet(player, ally, 'decapitate'),
            },
            "knife": {
                "stabHeart": () => knife(player, ally, 'heart'),
                "slitThroat": () => knife(player, ally, 'throat'),
            },
            "explosives": {
                "detonate": () => explosives(player, ally, 'detonate'),
            },
            "fists": {
                "beat": () => fists(player, ally, 'beat'),
                "overpower": () => fists(player, ally, 'overpower'),
                "strangulate": () => fists(player, ally, 'strangle'),
                "neckSnap": () => fists(player, ally, "neckSnap"),
            },
            "molotov": {
                "throw": () => molotov(player, ally, 'throw'),
                "miss": () => molotov(player, ally, 'miss'),
            },
            "bow": {
                "hit_heart": () => bow(player, ally, 'hit_heart'),
                "hit_limb": () => bow(player, ally, 'hit_limb'),
            },
        };
...
function handleBetrayal(player) {
    ...
    let msg1 = selectCombatEvent(player, ally)
    let eventMessage = `**${player.name}** has betrayed their ally **${ally.name}**!`;
...
async function main(channelId) {
    ...
    Object.values(players).forEach(async player => {
        ...
        if(!player.status.includes('dead') && ifAttack == "attack"){
            const event = selectCombatEvent(player);
            eventArr.push(event); // Push the result of combat event
        }
        else if (!player.status.includes('dead')){
            const event = selectEvent(player);
            eventArr.push(event); // Push the result of normal event
        };
    ...
    const alivePlayers = Object.values(players).filter(player => !player.status.includes('dead'));
    if (alivePlayers.length === 1) {
        embed.addFields({ name: "🏆 Winner", value: `${alivePlayers[0].name} wins!` });
        players = {};
        day = 1;
    } else {
        day++;
